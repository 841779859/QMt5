package com.example.asus.qmt5.Personalcenter.fragment;
...
import com.example.asus.qmt5.Personalcenter.fragment.basic.PictureSelectFragment;
...
//                mPictureIv.setImageBitmap(bitmap);
    ...
    class NetworkTask extends AsyncTask<String, Integer, String> {
        ...
        protected void onPostExecute(String result) {
            if(!"error".equals(result)) {
                Log.i(TAG, "图片地址 " + data.url+ result);;
                data.imageurl=data.url+result;
                Glide.with(mContext)
                        .load(data.url + result)
                        .into(mPictureIv);
    ...
    private String doPost(String imagePath) {
        ...
        // 这里演示添加用户ID
       builder.addFormDataPart("phone", data.ph);
        ...
        Request request = reqBuilder
                .url(data.url + "/uploadimage")
                .post(requestBody)
                .build();

        Log.d(TAG, "请求地址 " + data.url+ "/uploadimage");
        try{
